...
| JCP | Jubilant Cadista Pharmaceuticals Inc | Generics — US | United States | USD |
...
`deltaScore` is the movement in the entity's displayed score. It must equal the
implied dimension movement × that dimension's weight (§3.2), so the numbers stay
checkable: a +7 on `workingCapital` (weight 0.20) implies that dimension moving 35
points. Two constraints follow, and the data below satisfies both: no single action
may imply a movement beyond the dimension's headroom to 100, and the actions for
one entity and dimension may not cumulatively exceed it either.

| Entity | Action | Δ score | Dimension | Effort |
|---|---|---|---|---|
| JGL | Clear GR compliance on 11 consignment vendors | +7 | workingCapital | Low |
| JGL | Close 18 aged reconciliation breaks | +3 | risk | Medium |
| JGL | Apply matched receipts to open AR | +1 | workingCapital | Low |
| JGL | Resolve 12 high-risk manual journals | +2 | risk | Low |
| JBL | Clear SoD conflict on vendor creation and payment release | +5 | risk | Medium |
| JBL | Reduce approval cycle at Ambernath | +3 | operational | Medium |
| JBL | Settle 9 open close blockers | +2 | operational | High |
| JPS | Complete intercompany matching with JGL | +4 | workingCapital | Medium |
| JPS | Clear 7 aged reconciliation breaks | +2 | risk | Low |
| JCP | Close 2 open reconciliation breaks | +1 | risk | Low |
| JCP | Apply 2 unapplied receipts | +1 | workingCapital | Low |
| JHS | Clear 4 aged reconciliation breaks | +2 | risk | Low |
| JHS | Resolve GR timing on the sterile line | +2 | operational | Medium |
| JRP | Resolve the unauthorised vendor bank change | +3 | risk | High |
| JRP | File the overdue GSTR-3B return | +6 | compliance | Low |
| JRP | Clear 26 aged reconciliation breaks | +4 | risk | Medium |

The two JRP entries at the top of that list are **veto-clearing**, not weighted
movements. Resolving the bank change lifts the cap from 55 to 60, so the displayed
score becomes 58 (the raw score, since raw < 60). The UI must show these as
"removes cap" rather than as an ordinary dimension movement.
