...
**On subagents — mostly, do not.** They were tried on this project and one consumed
300,000 tokens over an hour without returning anything. The failure mode: asked to find
where something is *described*, a subagent reads files looking for paraphrases rather
than searching for strings, and a codebase this size will absorb an unbounded read
indefinitely.

Almost everything a subagent was wanted for is a `grep`. Locations, string occurrences,
which files changed — all answered in a second by the shell, in the main window, with
the result in front of you.

Do not use one for a general "quality pass" either. The gates already do deterministic
QA and have caught more real defects than any review would — white text on the amber
accent at 2.03:1, eight breadcrumb probes matching the rail, a table overflowing under
the drawer. An open instruction to find improvements produces plausible changes outside
the step, which is what rules 1 and 3 exist to prevent.

If you do use one, give it a closed question with a defined stopping point — "run this
grep and report the hits", not "find everywhere X is described".
...
The spec-old/ folder is the superseded original spec. Never read it. Nothing in it is
current, and its section numbers collide with spec-v2/.
...
7. Never edit the spec-v2/ files. If a task claims something is in the spec and it is
   not, say so and proceed.
8. If a task is ambiguous, ask before writing code.
9. You cannot see images. Never verify anything by taking a screenshot — you cannot
   read it back. Verify through the DOM: query the element, measure its rect, read its
   computed style, assert on the value. A DOM-rect probe told us the Group view was
   overflowing rather than overlapping; a screenshot could not have. Screenshots the
   verify scripts produce are for the human reviewer, not for you.

**If a step runs past about ninety minutes, stop and report where the time is going.**
Ten hours went into Step 33 and almost none of it was the feature — it was the same
harness race rediscovered three times. A step that is fighting its tooling is not a step
that needs more persistence; say so and let me decide.

Before reporting, check these four. Not a general review — these four, because each is
a failure this project has actually produced:
  1. Does any figure you changed contradict the same figure on another screen?
  2. Did any rationale reach the screen? Describe, never persuade (spec-v2/rules.md §10.1).
  3. Could any assertion you wrote pass for the wrong reason — a loose matcher, a string
     the rail also carries, a loop that can emit zero checks?
  4. Is every number you rendered present in the dataset module?
...
```
Read spec-v2/agents.md (§15.6 §15.3), spec-v2/data-core.md (§7.8.1).

Task: extend control-of-the-control to your own agents.

0. One carry-over from Step 20. Your flag 1 was right that §15.4 and §15.3 give
   different end states, and both are correct — they are different scenarios. 138 is
   agents applied to today's exception volume; 114 is causes eliminated first, then
   agents on the smaller residue. §15.3 now says so explicitly.

   The screen currently shows both with nothing distinguishing them, which reads as an
   error on the one screen whose argument is that the two levers compound. Label them:
   "agents alone, on today's volume — 138 per 1,000" against "causes eliminated first,
   then agents — 114 per 1,000".
...
## Step 21b — Sweep the drill script

Your flag (p) found a probe that passes by matching the rail rather than the breadcrumb.
That is the shape Step 18d swept for — but that sweep covered the 21 files in `tests/`
and never touched `verify-drills.mjs`, which now carries 630 checks. More assertions
than the unit suite, entirely unswept.

Do this before Step 22 adds more checks to the same file.

```
Task: sweep verify-drills.mjs and verify-theme.mjs for checks that pass vacuously.
No app changes.

1. The specific fault you found: a probe scoped to the whole document, or to a nav
   containing "Group", matches the RAIL — which lists every screen label — instead of
   its intended target. Any breadcrumb, header or section check written that way passes
   for the wrong reason, and keeps passing after its target is deleted.

2. Go through every probe in both scripts:
   - scope each to its actual container — nav[aria-label="Breadcrumb"], main, the
     section under test — never the document
   - check that any string it asserts on is unique to that container, not something the
     rail, palette or breadcrumb also carries
   - flag any check that would pass against a page where its target is missing

3. Fix them in place, and report each one: what it claimed to check, what it was
   actually matching, and what you scoped it to.

4. Do not add coverage. The count may fall if a check turns out to be testing nothing;
   say so rather than replacing it with a new one.

Run all four gates.
```

**Check:** the report should name specific probes. Step 18d returned zero findings on
`tests/` with sound reasoning; this file has already produced one by accident, so zero
here would be worth questioning.
...
0. Close the two items you flagged in Step 21b before adding checks to that file:

   a. Delete the three check(name, true) milestone markers. They cannot fail, and six
      checks that cannot fail inflate a count we quote. A check that always passes is
      not coverage; removing it makes 630 mean what it says. Report the new count and
      why it fell.

   b. Fix the FILL_AUDIT silent pass. A loop that emits zero checks and reports success
      is the same vacuous-pass shape as the breadcrumb probes, one level up — a whole
      route's fills could go unaudited and the run would still be green. A minimum-count
      assertion is added coverage, but it is coverage of the harness rather than the
      app, which is the exception worth making.

   c. Optional, low priority: the two-condition probes work only because the rail
      happens not to contain "JGL". That is true today and nothing guarantees it stays
      true. Scoping them to nav[aria-label="Breadcrumb"] like the other eight costs
...
0. One carry-over from Step 22, flag 6. Deccan shows CREDIT BLOCKED with an agent
   record beneath saying the block was released. That reads as a contradiction on
   screen, and the reconciliation sentence is in DEMO.md — your narration, not the
   product. Anyone clicking without you sees a bug.

   It is the same fault as "cleared" in Step 20: an outcome verb claiming execution
   when only a decision happened. Fix it in the record, not with a caption — the
   outcome reads "Release approved · not yet posted" or equivalent. Then there is no
   contradiction to explain.

   Audit the other outcome strings for execution verbs while you are there. Anywhere
   the screen shows both SAP state and an agent record, the record must not claim the
   state has changed. Where no SAP state is shown alongside, the existing wording is
   fine — this is a reconciliation problem, not a systemic rewording.
...
## Step 23b — Get the gates green on the branch

Run this first on `design/clay-graphite`. Twenty-six checks are failing, and building
Phase 3 on a red harness means you cannot tell your own failures from the ones already
there — while every new screen has to register in the two scripts that are broken.

```
Read spec-v2/rules.md (§10).

Task: return verify:theme and verify:drills to green on this branch. Palette and
layout changes are the designer's; do not revert them.

1. First, make both scripts wait for the app before auditing. Wait for the rail or
   nav[aria-label="Breadcrumb"] to exist rather than assuming the page is ready. The
   logo intro on first load is producing blank captures and blank captures fail
   everything. This is the same class of fix as the scrollIntoView and settle-line
   patches earlier in the build, and it makes the scripts immune to it permanently.

   Re-run after this. Whatever still fails is real.

2. Then triage what remains, in this order:

   a. "toggle: click flips theme without reload" — the serious one. If theme switching
      is broken, the light theme is unreachable and half the product is unusable.

   b. hover / keyboard focus / palette scrim / palette box-shadow — four failures that
      almost certainly share one cause. src/index.css references CSS variables set by
      applyTheme(); if tokens.ts renamed or removed any, the stylesheet is naming
      variables that no longer exist. Fix the root, not the four symptoms.

   c. "contrast: no non-exempt in-situ text pair below AA 4.5" — a genuine regression.
      The output names the pair and the ratio. Fix the token value, not the usage, and
      check the change holds in BOTH palettes.

   d. The seven "expected colors present" routes. A recolour legitimately changes which
      token each element uses, so these lists go stale — that is the check working, not
      failing. Update them, but verify each route individually rather than blanket
      updating. That check only became real in Step 21b when it was scoped to <main>,
      and a blanket update would undo that.

3. Two defects visible on screen that the gates do not catch:

   a. The Group view table has collapsed — dimension labels printing over each other
      ("Operational" over "Service & attribution") and money columns running together
      as a single string. First screen anyone opens.

   b. Remove the "Switch to Liquid Logo Alternative" toggle. It is pinned bottom-right
      on every screen and cannot be in a client build.

4. The logo intro: gate it to first load only, under a second, and skippable. It must
   never block a route change — a controller clicking between twenty-four screens
   should not watch an animation each time.

Report each failure, its cause, and what you changed. Where a check was stale rather
than wrong, say so.
```

**Check:** all four gates green, then `git switch main && git merge design/clay-graphite`
before starting Phase 3 — or stay on the branch if you would rather merge once at the
end. Either is fine; building on red is not.

---

# Phase 3 — Record to report

Steps 24 to 28. **This work lands on the `design/clay-graphite` branch.**

Two things before Step 24.

**Run Step 23b first.** It is immediately above this section. The branch is red, and
building four new screens onto a red harness means you cannot tell your failures from the
ones already there.

**Match the branch's design grammar, not the spec's screenshots.** The clay-graphite work
introduced patterns the spec does not describe — expandable entity rows, the Cash
attribution canvas, different chrome. Read those screens before building and follow what
is there. §10.1 still governs copy; §10's token and radius rules still govern colour.

---

## Step 24 — R2R data layer and cockpit

```
Read spec-v2/r2r.md (§16.0 §16.2 §16.8), spec-v2/rules.md (§13.1).

Task: the data layer and the third process cockpit.

0. Two carry-overs from Step 23b:
   a. Tokenise the literal '#FFFFFF' at TopBar.tsx:84. No gate flags it because it is
      not text, but §10 says no literal hex in components — and the rule exists so a
      palette change cannot miss anything. A decorative dot that survives one recolour
      by luck will not survive the next.
   b. Correct the PROGRESS.md line that records the QA pass as "Step 24". That work was
      Step 23b; this is Step 24, and two of them will confuse a later session.

1. Extend ProcessKey to 'R2R'. Add the eight stages in §16.2 to stages.ts, per entity,
   following the §7.25 pattern — in-flight and in-exception, percentages derived.

2. The R2R cause taxonomy in §6.1 has been unused since Step 1. Wire it now: source
   data, journal, reconciliation, intercompany, master data, interface, close
   dependency, judgement. Root cause must work for R2R as it does for P2P and O2C.

3. Build /entity/:code/r2r — the cockpit shell with the stage flow. Panels come in
   Step 26.

4. Register it fully: routes x4, palette, verify-drills, verify-theme.
...
## Step 25 — The close calendar

```
Read spec-v2/r2r.md (§16.1 §16.3).

Task: the close calendar, as a capability rather than a status read.

0. One carry-over from Step 24, judgment call 3. 'master data' returning two palette
   rows — a cause and an agent — is a product problem, not a test problem. Both names
   are spec-derived so neither is wrong, but a user cannot tell them apart.

   Fix it with meta text rather than renaming: the cause row reads "cause · R2R", the
   agent row "agent · shared", the way screen rows already carry their meta. Do it as a
   pattern, not a special case — 'reconciliation' is about to exist as a cause, a stage
   and an agent, and 'intercompany' likewise.

1. Read §16.1 first. The scope boundary has moved and you should understand why before
   building: Jubilant has no close tool, so "we read close status from your tool" reads
   from a spreadsheet. Owning the calendar is the job. But journal posting to SAP is NOT
   in this step — it is staged and priced separately, because it puts the platform in
   ICFR scope.

2. Tasks per entity from §16.3, reconciling to the pinned close percentages in §7.2.
   Assert that reconciliation.

3. Dependency-aware. A blocked task names what blocks it and who owns that. A checklist
   that cannot show dependencies cannot produce a critical path, and the critical path is
   the point.

4. Predicted close date against committed, from the critical path through open tasks. Use
   the §16.3 table; assert that predicted slippage orders with the close percentage.

   This delivers something the deck has claimed since slide 2 and the product has never
   had — that it anticipates rather than reports.

5. Escalation on a timer to the owner's escalation contact, logged. Reuse the exception
   worklist's escalation mechanism; do not build a second one.

6. Sign-off is STATUS ONLY. Read certification state from the reconciliation platform and
   display it. Preparer/reviewer workflow with evidence attachment is phase B.
...
## Step 26 — Balance sheet integrity and the four panels

```
Read spec-v2/r2r.md (§16.4 §16.5).

Task: the headline figure and the four panels beneath it.

1. Balance sheet integrity from the six components in §16.4, weighted. Compute it, never
   store it. Assert two properties: the index matches the pinned values, and it orders
   with the risk dimension.

   It feeds the risk dimension; it does not become a seventh. §3.2's weights are
   contractually fixed and rendered on three screens.

2. Reconciliations panel — status, breaks by age band, evidence attached or not. Sourced
   from the reconciliation platform; tag the source.

   Note: Step 24 correctly dropped ageingBarAlt from the r2r-cockpit presence list
   because the shell had no ageing bars. This panel adds them, so re-add it — and check
   the rest of that route's list against what now renders. That check is only meaningful
   because Step 21b scoped it to <main>; leaving it stale would quietly undo that.

3. Journal risk panel — the POPULATION, scored on the seven flags in §16.5. Not the
   exceptions; Risk & control already shows what breached. High-risk counts must match
   highRiskJEs in §7.2 — assert it.

   Note on screen that preparer-equals-approver and outside-hours detection depend on SAP
   change documents. If that extract is unavailable the panel degrades, and the product
   should say which flags are missing rather than silently omitting them.

4. Intercompany — matched against unmatched by counterparty, ageing, netting opportunity.
   JPS highest at Rs 5.8 cr. The Ingrevia netting row belongs here.

5. Accruals and provisions — exposure, adequacy against actual utilisation, and
   prior-period accruals that did not reverse. Auto-reversal failure is a real source of
   misstatement and is rarely monitored.
...
## Step 27 — Four more agents

```
Read spec-v2/r2r.md (§16.6), spec-v2/agents.md (§15.2 §15.2.1).

Task: extend the roster from eighteen to twenty-two.

0. Two carry-overs from Step 26:
   a. Fix the two stale comments in verify-drills.mjs — line 575 ("Until Step 26's
      panels land…") and line 1684 ("no new fill pairs are introduced"). Both panels
      landed and a fill pair was added. A comment that has stopped being true is a
      small lie the next reader has to detect.
   b. The union-not-sum reading of highRiskJEs is now written into §16.5, so it no
      longer lives only in a test. No code change needed — just be aware the invariant
      is documented if you touch the flag counts.

1. Add agents 19 to 22 per §16.6. Nothing in §15's rules changes — same delegation model,
   same decision record, same supervision.

2. Note what none of them do: post a journal. Reconciliation clearing proposes above its
   cap. Intercompany matching never posts the netting entry. Accrual reversal reverses
   only. Cut-off surveillance is advisory. That is the same management-versus-execution
   line as §15, and a journal workflow would make all four posting agents.

3. Build cut-off surveillance first. Cut-off is where most audit adjustments originate,
   it is almost never monitored during the month, and it is preventive — eight of
   twenty-two now.

4. Update the coverage strip: R2R stops reading "not yet covered" and gains its eight
   stages with agents positioned. That gap is currently the most visible hole on the
   agents screen.
...
## Step 28 — Boundary, trace and the walk

```
Read spec-v2/r2r.md (§16.1 §16.7), spec-v2/ui.md (§8.10).

Task: reconcile the rest of the product to the new scope.

0. Three carry-overs from Step 27:

   a. The declined line renders only on ExceptionDetail. §15.1.2 makes it a property of
      every decision record, and the agent's own action log on /agents is where a
      controller reviews an agent across items — which is exactly where "what did it
      decline to do" earns its place. Add it to ActionRow. Step 27 was right to fix the
      assertion rather than the code; the gap is in the UI.

   b. Confirm the risk-control "without review" count of 5 by reading the accessor, not
      by the run passing. A deduction and an assertion that share an error confirm each
      other.

   c. Check the coverage strip no longer says R2R is "not yet covered". Four agents are
      positioned there now, one of them live. If that note survives it is false — and it
      sits on the screen whose whole argument is that the gaps are named honestly.

1. §1's exclusion list drops from three items to two. Close orchestration is no longer
   excluded — it is staged. Update anywhere the product states the old boundary.

   Find it with grep, not by reading files:

       grep -rn "close orchestration\|Close orchestration" src/ tests/ DEMO.md README.md
       grep -rn "consumed, not produced\|does not orchestrate" src/ tests/ DEMO.md README.md

   Then edit the hits. Do NOT delegate this to a subagent — "find everywhere the product
   states the old boundary" is an open-ended read that will consume the codebase looking
   for paraphrases.

2. The cross-process trace in §8.10 ends at a close-card anchor today. Land it on the R2R
   cockpit instead, so the chain — goods receipt, blocked invoice, understated accrual,
   exposure at close — resolves on a real screen.

3. Mode-awareness: pre-close readiness, close cockpit and BAU become three views of a
   screen that owns the calendar. Verify all three render on the R2R cockpit.

4. Add DEMO.md beats: the close cockpit showing JRP predicted at day 9 against a
   committed day 6, and the journal risk panel.

5. Refresh PROJECT-MAP.md. Four new screens and a new data module have landed since it
   was last written. Get the inventory from git rather than by reading files:

       git diff --stat <last-map-commit>..HEAD -- src/

   Then update the map's file list and screen inventory from that. The stack, tokens and
   conventions sections have not changed.

Run all four gates, and report the final counts.
```

---

## Step 29 — Truncation, and connecting the close calendar

Two findings. The first is systemic and affects every table in the product.

```
Read spec-v2/ui.md (§8.4 §8.9), spec-v2/r2r.md (§16.3 §16.6).

Task: fix truncated cells everywhere, then connect the close calendar.

PART A — truncation with no recovery

1. DataTable's cell ellipsis was added in Step 7 to fix a column collision, and nothing
   was ever added to get the hidden content back. Every table in the product has
   carried it since. On the close calendar it hides the actionable half: "owned by
   K. Me…" loses the owner, "sent 09:40 to Entity con…" loses the recipient.

2. Three fixes, not alternatives — do all three:

   a. A title attribute on any cell whose content is truncated. One change in
      DataTable, native tooltip, applies to every table at once. This is the floor.

   b. Row expansion on click, using the pattern the Group view already established —
      the row opens in place with the full content. Same component, same grammar.

   c. On the close calendar specifically, fix the layout rather than only the symptom.
      "Blocked by" and "Escalation" hold sentences, not values, and two narrow columns
      forced to carry prose is the actual defect. Either widen them into one column or
      move the blocker beneath the task name as a sub-line.

3. Audit every other table for the same problem and report which columns truncate. Do
   not fix them beyond (a) and (b) unless a column is unreadable — report and stop.

PART B — the close calendar connects to nothing

4. §8.4 says every figure drills and this screen was built without it. Wire four:

   a. Blocked-by → the blocking task. You already computed the dependency graph for the
      critical path; it is currently usable for prediction and not for navigation.
   b. Task → the exceptions behind it. "Intercompany matching — JBL pair" reaches the
      intercompany panel on the R2R cockpit.
   c. Escalation → the escalation record. What was sent, to whom, answered or not. The
      worklist already has this model; reuse it.
   d. Task → agent, which is the one that matters.

5. On (d): four R2R agents exist whose scopes map onto these task rows almost exactly —
   reconciliation clearing, intercompany matching, accrual reversal, cut-off
   surveillance. A row reading "Intercompany matching — JBL pair, BLOCKED" sitting on a
   screen with an agent of that name and no link between them is a gap a client will
   spot.

   Add an agent lane to the task table, the same four states as the worklist: agent
   working with its timer, escalated to you, agent resolved, needs a human. Where no
   agent covers a task, the lane is empty — do not invent coverage.

   This is what turns the close calendar from a project plan into a close being worked
   by a workforce, on the screen where a controller spends close week.

Run all four gates.
```

**Check:** hover any truncated cell and the full text appears; click a blocked row and
it reaches what is blocking it. If the agent lane shows coverage on every row, something
was invented — four agents cannot cover 214 tasks.

---

## Step 30 — The drill audit, re-run against the whole product

Step 16 audited every number for drill-through and did it well. It ran at Step 16 —
before the Finance Service Desk, the agents, touch economics, cash attribution, the R2R
cockpit and the close calendar existed. Every screen built since is a potential dead end
from the perspective of every screen built before it, and several figures were correctly
tagged read-only at the time because their target had not been built yet.

```
Read spec-v2/ui.md (§8.4 §8.6.1), spec-v2/r2r.md (§16.3).

Task: re-audit drill-through across all twenty-four screens. Fix the breaks; report
everything.

1. The known one first. Close % on Entity health carries "read-only · source: close
   tracker" from Step 16, when no close screen existed. Two now do. The figure is live,
   drillable and has a detailed view behind it — the tag is false. Drill it to the close
   calendar, which holds the same 78% as 167 of 214 tasks with the blockers named. Same
   for the Group view close card.

2. Then re-check every figure previously tagged read-only or left unlinked because its
   target did not exist. Known candidates:
     - provision adequacy, tagged read-only in Step 5b — the accruals panel now exists
     - cause elimination forward links, wired to P2P only — the R2R taxonomy now exists
     - O2C causes, sent to the collection stage anchor in Step 14 — still correct, there
       is still no O2C worklist. Confirm and leave it.

3. Audit the screens that have NEVER been through this. Step 16 predates all of them:
   Finance Service Desk, Agents, Agent detail, Touch economics, Cash attribution, R2R
   cockpit, Close calendar, PO detail, Commitments watch.

   Cash attribution needs particular attention — it came from the design branch and has
   never been checked against §8.4 at all.

4. For every figure on every screen, one of three must be true, and report which:
     - it drills, and the target holds the same figure with more detail
     - it is genuinely terminal (a leaf value, an ID, a date)
     - it is read-only from an external system, tagged with the source

   A figure tagged read-only whose target now exists is a defect, not a tag.

5. Check the reverse direction too: does every screen have a way IN from somewhere a
   controller would naturally be? A screen reachable only from the rail is not wired
   into the product, it is parked beside it.

Report the full inventory — what drills, what is terminal, what is read-only, and every
tag you corrected. Fix the breaks; do not add links for their own sake.

Run all four gates.
```

**Check:** no figure carries a read-only tag whose target now exists. And clicking Close
% on Entity health lands on the close calendar showing the same 78%.

---

## Step 30b — The inventory, and the screens that were never audited

Step 30 fixed the known read-only tags and found a real defect on the way — the Group
view close card contradicted the close calendar because a legacy `closeProgress` dataset
had survived Step 25. Items 3 to 5 were not attempted, and they are the half that finds
breaks you have not noticed yet.

```
Read spec-v2/ui.md (§8.4 §8.6.1).

Task: audit the screens Step 16's check predates, and produce the full inventory.
Report before fixing.

1. These nine screens have never been through a drill audit. Step 16 predates all of
   them:

     Finance Service Desk · Agents · Agent detail · Touch economics
     Cash attribution · R2R cockpit · Close calendar · PO detail · Commitments watch

   Cash attribution needs the most attention — it came from the design branch and has
   never been checked against §8.4 at all. It is also the most visually impressive
   screen in the product, which makes a dead end there expensive.

2. For every figure on each, one of three must be true. Report which, for each figure:
     - it drills, and the target holds the same figure with more detail
     - it is genuinely terminal — a leaf value, an ID, a date
     - it is read-only from an external system, tagged with the source

   A figure tagged read-only whose target now exists is a defect, not a tag. Step 30
   found four of those.

3. Then the reverse direction, which nothing has ever checked: does every screen have a
   way IN from somewhere a controller would naturally be? A screen reachable only from
   the rail is parked beside the product, not wired into it — and in a demo you would
   have to navigate to it deliberately, which shows.

   Touch economics and cash attribution are the likely candidates. Report every screen
   whose only entry is the rail or the palette.

4. Report the inventory first and wait. Do not fix anything in this pass beyond an
   outright contradiction of the kind Step 30 found — a figure that disagrees with the
   same figure elsewhere. Those are defects; everything else is a decision I want to
   make from the list.

Do not add links for their own sake. Some figures are genuinely terminal and forcing a
drill onto them adds noise.
```

**Check:** the report should be an inventory, not a changelog. If it comes back with
twenty fixes applied and no list, the audit was skipped in favour of the fixing.

---

## Step 31 — Reconcile the O2C label, and the small drills

Decisions from the Step 30b inventory. The Cash attribution rebuild is Step 32.

```
Read spec-v2/data-detail.md (§7.25), spec-v2/ui.md (§8.4).

Task: fix the label, fix a real per-entity defect, wire four links.

1. The O2C "overdue AR" contradiction is a LABEL problem, not a number problem. Three
   figures, all correct, all different things:

       Rs 12.4 cr   AR over 90 days              an ageing bucket
       Rs 18.8 cr   sum of six O2C cause values  cash stuck, by cause
       Rs 20.6 cr   overdue AR                   everything past due

   Cash attribution is summing CAUSES. Its P2P side reconciles to blocked AP only
   because that figure happens to be both a pool and a cause sum — coincidence, not
   design. Relabel the O2C pool "O2C causes" or "revenue at risk". Do NOT re-pin the
   cause values and do NOT move 20.6; both would break something true to fix something
   that is not wrong.

2. The latent issue you flagged IS a defect and is bigger than the label.
   o2cKpis.overdueArCr is 20.6 for all six entities. Every other O2C figure went
   per-entity in Step 25 and this one was missed — JCP currently shows the same overdue
   AR as JRP.

   Derive it per entity the way §7.25 derives the other O2C figures: overdue AR is
   receivables buckets 3 + 4 + 5 from §7.31, which are already per-entity. Assert the
   tie for all six.

3. R2R cockpit: the header "Open breaks" and the panel "Overdue breaks" are the same
   figure (reconAgedBreaks) and only one links. Make both link.

4. Touch economics: link the Entity column to EntityHome and the Cause column to
   RootCause. Those same names drill on every other screen.

5. Touch economics is rail-only with nothing linking in. It is the commercial screen —
   a controller reaches it in a conversation about cost, not during close week — so
   rail-only is defensible, but add it to the CommandPalette and link to it from the
   Agents roster summary. A screen with no palette entry is invisible to search.

Leave these as they are, per your own reasoning, which is right in each case: the
coverage strip chips (a legend, not a set of links), Service Desk entity codes (a
group-scoped screen), the commitments aggregates (no single cost centre owns them), and
the close calendar sign-off read-only tag (no sign-off detail screen exists).

Run all four gates.
```

**Check:** JCP and JRP show different overdue AR. Cash attribution no longer calls a
cause sum "Overdue AR".

---

## Step 32 — Cash attribution v2, alongside the original

Section E of the Step 30b inventory found that Cash attribution is an iframe loading a
static HTML file: every figure hardcoded, its own palette and fonts, rounded corners
against the zero-radius rule, `Math.random()` in a decorative layer, and no drill links.
It agrees with the dataset today by coincidence.

**Build v2 as a second screen. Do not replace the original.** Both stay routable so they
can be compared side by side, and the decision about which survives is not yours to make
in this step.

**The design is good and must survive unchanged.** The function axis, the lens list and
the fix-status colouring are better than anything in the spec. It is the implementation
that is a mockup.

```
Read spec-v2/rules.md (§10 §10.1 §13.1), spec-v2/ui.md (§8.4).

Task: build /cash-attribution-v2 as a React screen inside the product. The original
stays exactly as it is.

THE COMPARISON MUST BE FAIR. Same structure, same function axis, same lens list, same
Sankey, same panel arrangement. Do not improve the layout, reorder anything, or add
elements. If v2 also redesigns, nobody can tell which change caused which difference —
and the comparison stops being evidence.

What changes is implementation only:

1. Every figure from src/api/ — causes.ts, causeBacklog.ts, misc.ts, entities.ts. No
   hardcoded values anywhere. The original matches the dataset by luck, and figures have
   changed roughly every third step of this build.

2. The app's own tokens and type, whatever theme is currently active. The original's
   palette (#e8b34b, #8eafd1), fonts (SF Mono, -apple-system) and rounded corners are
   outside the design system. Zero radii except StatusDot, per §10. It must read as the
   same product as the other twenty-three screens, in both themes.

3. No Math.random(). Drop the decorative starfield or render it deterministically from a
   seed. No figure depends on it, but the rule has no exceptions.

4. Wire the drills — the biggest §8.4 gap in the product, on its most visible screen.
   The cause keys already match the taxonomy exactly:
     - a cause band drills to RootCause for that cause
     - a register row drills to Cause elimination
     - a cash opportunity drills to Working capital
     - the two pools drill to the P2P worklist and the O2C cockpit

5. Entity-scoped and mode-aware. The original hardcodes "JGL" and "DAY 4 OF CLOSE" while
   the nav treats it as group-scoped and the app defaults to pre-close. Pick one — per
   entity like the cockpits, or genuinely group-level — and be consistent.

6. Register v2 properly: rail (beside the original, labelled so they are
   distinguishable), CommandPalette, verify-drills, verify-theme. Note that the ORIGINAL
   has no palette entry and is audited by neither script — leave that as it is for now;
   it is part of what the comparison shows.

7. Report what you could not reproduce. If any part of the original's design does not
   survive the token system — a colour with no equivalent, a radius the rules forbid —
   say so rather than approximating silently. That list is the honest cost of bringing
   it inside the system, and it is what the comparison is for.

Run all four gates.
```

**Check:** open both screens side by side. The layout should be indistinguishable; the
colours, the fonts and the drills should differ. Then change a cause value in causes.ts —
v2 moves, the original does not. That single test is the whole argument.

---

## Step 32b — Restore the original, and fix the Sankey

Two problems. The first is that Step 32 replaced the original screen instead of building
alongside it; the second is that the visualisation does not behave.

```
Read spec-v2/rules.md (§13.1), spec-v2/ui.md (§8.4).

Task: restore the comparison, then diagnose the Sankey properly.

1. Step 32 replaced /cash-attribution rather than building alongside it. The original
   was the baseline for a side-by-side and there is nothing to compare against now.

   Recover it from git and restore it as a SECOND screen:

       /cash-attribution-original    rail label "Cash attribution (original)"
       /cash-attribution             rail label "Cash attribution"  — the React build

   Naming them that way says which is which without a legend, and survives the decision:
   when one is retired the other is already correctly named.

   RESTORE IT EXACTLY AS IT WAS. Same iframe, same hardcoded figures, same palette, same
   fonts, same rounded corners, same starfield. Do not tidy it on the way back — and
   that includes reverting Step 31's "Revenue at risk" relabel on this copy. A
   partially-converted original proves nothing; the whole point is to see what v2 does
   differently, and the label was one of those things.

   Register the original in routes and the rail only. Leave it out of the palette and
   out of both verify scripts, exactly as it was — that absence is part of what the
   comparison shows.

2. Then the behaviour. The reported symptoms — highlight reverts to a selected path,
   works for missing-GR but not other causes, does not follow the lens — are consistent
   with TWO selection states competing: a default selection plus a hover or lens
   highlight that is overwritten when the component re-renders.

   Do not patch the symptom. Find out how many pieces of state drive what is
   highlighted, and report them before changing anything. If there are two, say which
   one wins and when.

3. Verify the geometry is real, which is the load-bearing question. Ribbon width must
   derive from the cause value. In the current render, missing-GR at Rs 6.4 cr is very
   wide while pricing disputes at Rs 5.4 cr is a thin sliver — those are 34% and 29% of
   their pool and should be close to the same width.

   If the band geometry is still the reference prototype's fixed layout with real
   numbers written over it, the rebuild is incomplete regardless of where the figures
   come from. Report what you find.

4. The lens must drive the highlight. Selecting "caused outside the owning team" should
   highlight that set; selecting another lens should replace it, not fall back to a
   default. Nothing should be auto-selected on load unless the design intends it — and
   if it does, that selection must clear when a lens is chosen.

5. Move the "6 d" close-lens threshold into the dataset. It is a business rule — the
   cut-off risk threshold — not a layout constant. Layout constants are px values;
   this is a number a controller could argue with, and it is the third business figure
   in this build to end up as a literal.

Report findings for items 2 and 3 before fixing. Run all four gates.
```

**Check:** change a cause value in causes.ts and the ribbon width changes visibly. Select
each lens in turn and confirm the highlighted set changes and stays changed.

---

## Step 37 — Requisitions becomes a drill, and a process-filter sweep

Two things, the second prompted by what Step 36 found.

```
Read spec-v2/upstream.md (§18.0), spec-v2/ui.md (§9.1).

Task: move Requisitions out of the rail, then sweep for the bug class Step 36 hit.

1. Requisitions is a stage drill, not a rail entry — the same as Commitments watch. Both
   belong to the P2P funnel and neither is somewhere a controller navigates to directly.
   Step 35 put it in PROCESS because that was the only slot the adjacency tests left
   free; the answer was that it does not belong in the rail at all.

   Remove the rail entry. Keep the route, the breadcrumb, the palette entry and the
   P2P Requisition-stage drill. PROCESS then holds the three cockpits and the close
   calendar, which is the right shape.

2. Then the sweep. getPlantDetail filtered exceptions by entity and plant but not
   process, so O2C rows leaked into plant pages — Nanjangud showed 8 open items instead
   of 3. It had been correct since Step 11 and became wrong the moment an O2C exception
   dataset existed. The counterparty vendor-tie comparator had the same fault.

   That is a class, not two incidents: an accessor that filters by entity without
   filtering by process was correct when written and is wrong now.

   Go through every accessor in src/api/index.ts that reads exceptions. For each, decide
   whether it should be process-scoped, and report:
     - the accessor
     - whether it filters by process today
     - whether it should
     - what it currently returns that it should not

   Fix the ones that are wrong. Where an accessor is deliberately cross-process — the
   root cause register spans both — say so rather than adding a filter it should not
   have.

3. Report the sweep before fixing anything beyond an outright leak of the Nanjangud kind.

Run all four gates.
```

**Check:** the rail's PROCESS group holds four items. Every exception accessor either
filters by process or has a stated reason not to.

---

## Step 37b — Harden what is currently safe by coincidence

Step 37's sweep found three readers that are correct today for reasons nothing enforces.
Each would break silently, and none would fail a gate.

```
Task: add the filters, pin the absence. No screen changes.

1. agents.ts — byReason(key), causePrecedent(x), rows(key) filter on reason key only.
   They are safe because P2P and O2C cause keys happen to be disjoint. Nothing enforces
   that: §6.1 lists "pricing" in both taxonomies, and only the current key naming keeps
   them apart. Name an O2C key "approval" or reuse "master data" and a P2P row cites an
   O2C precedent with nothing failing.

   Add the process filter.

2. runNextAgentCycle is entity-scoped and correct because no agent has ever acted on an
   O2C row. That stopped being a structural guarantee when Step 36 introduced the O2C
   worklist and its agent lane. Scope it by process.

3. Pin the vendor-side absence. Only the plant count is currently asserted. O2C rows
   carry vendor = customer, so without the process filter a customer name appears as a
   vendor — and that is the leak a demo would surface, because someone clicks a vendor
   page. Assert that no O2C counterparty name appears in the vendor list.

4. While you are there: is there any other place where correctness rests on two key sets
   being disjoint, or on a dataset being empty? Both were true when written and are the
   same fault shape. Report what you find; fix only what leaks.

Run all four gates.
```

**Check:** rename an O2C cause key to collide with a P2P one in a scratch edit. Something
should fail. If nothing does, the filters are not doing the work.

---

## Sequencing notes — phase 3

Step 25 is the one that changes the commercial conversation. A close calendar the
provider owns, with a predicted landing date, is the difference between running the close
and reporting on it.

Step 26's journal risk panel is the differentiated half of R2R and needs no write-back —
it reads a population SAP already holds. If the change-document extract turns out to be
out of scope, build the panel anyway and show which flags are unavailable. An honest gap
is better than a quiet omission.

---

# Phase 4 — Root causes

Steps 33 onward. The register comes first, because it is the missing element today and
everything else is rewiring around it.

---

## Step 33 — The root cause register

```
Read spec-v2/rootcause.md (§17), spec-v2/rules.md (§13.1).

Task: the register data layer and screen. Do not touch the cockpit drills yet.

1. Read §17.3 before writing any data. The four states are the argument of the screen
   and the line a client will test. Resolving an instance is NOT fixing a cause —
   chasing a QC approver clears that item and the next receipt hits the same wall. That
   entry is identified, or in progress if something structural is underway. Never
   fixed-at-source.

   The four eliminated entries must be genuinely structural: a master data correction, a
   changed invoicing arrangement, a tolerance reset, a delegation fix. Nothing that reads
   as "we dealt with it".

2. Build the register: three root causes per §6.1 cause, twelve causes, thirty-six
   entries, distributed per §17.7 — 4 eliminated, 7 fixed at source, 8 in progress, 17
   identified.

3. The ties in §17.6 are the load-bearing constraint. Root cause values sum to their
   parent cause value; item counts sum to the parent cause pool from §7.20. Missing GR is
   Rs 6.4 cr across 111 invoices, so its three root causes sum to both. Assert for every
   cause.

4. Build /root-causes — group-level, filterable by process, entity, status and owner.
   NOT split per process: "consignment invoicing structure" produces missing GR in P2P
   and can produce billing errors in O2C, and that repetition is only visible in one
   list. The filter is also the demo mechanic.

5. Cause elimination merges in. Its entries become register entries, its status counts
   become the register's, its mechanism chart stays. Delete the old Cause elimination
   screen and the old Root cause screen; the §6.1 taxonomy survives as a filter, not a
   page.

   Leave the cockpit stage drills alone this step — they point at the old Root cause
   screen and will break. Note which ones and fix them in Step 34.

6. Some root causes have no agent (§17.8). Moving consignment vendors to GR-based
   invoicing is a commercial negotiation. Showing which causes cannot be automated is the
   honest counterpart to the touch rate — do not give every entry an agent.

7. Traversal (§17.9): one line per affected item, expandable on a few examples only. Do
   NOT build any screen explaining what a knowledge base is, how traversal works, or
   showing a sample KB — that page is owned elsewhere. Build the path on an item, as
   data. Not the capability explanation.

Register fully: routes x4, palette, verify-drills, verify-theme.

Run all four gates.
```

**Check:** click each of the four eliminated entries. Zero open items behind every one.
If any shows open items, it is fixed-at-source and mis-stated.

---

## Step 33b — Stop rediscovering the same harness bug

Step 33 took ten hours and 520,000 tokens. Almost none of it was the register — it was
three `verify-drills.mjs` bugs and two loose test assertions. That script has now caused
six multi-hour debugging sessions in this build, and every one is the same fault: it
assumes state transitions are instant, and each check rediscovers that they are not.

```
Read spec-v2/rules.md (§13).

Task: give verify-drills.mjs the helpers its checks keep reinventing. No app changes.

1. Audit every navigation and interaction in the script. Six recorded failures share one
   cause — a check acting before the app has committed:

     Ctrl+K fired before the page settled
     a coordinate click landing on layout that had grown
     blank captures because a splash was still up
     a pathname compare that could not see a query drop
     a URL flipping ~130ms before React committed the filter reset

2. Build four helpers and route every check through them:

     goTo(url)            navigate, then wait for the app to have committed —
                          the target route rendered, not just the URL changed
     clickOn(selector)    scroll into view, wait for actionability, click, wait for
                          the resulting commit
     waitForText(scope, s) poll within a scope, with a hard timeout that fails loudly
     readAttr(sel, attr)   attribute values, not getBoundingClientRect — rects on
                          elements inside <defs> always return zero, which cost a
                          session on its own

   Every helper times out and fails with the selector it was waiting for. A silent hang
   is worse than a failure.

3. Delete every hand-rolled probe that duplicates one of the four. Step 33 lost time to a
   probe with its own coordinate bug — a bug that could not exist if the probe did not
   exist.

4. Two escaping traps, both of which have cost a session. Write them as comments at the
   top of the file:
     - `\d` inside a direct regex literal matches a literal backslash-d. Use `\d`
       only in strings passed to `new RegExp()`.
     - Never nest template literals inside `evaluate()`. Build the string node-side and
       inject it with `JSON.stringify`.

5. Report which checks changed and confirm the count is unchanged. This is a refactor of
   how checks run, not of what they assert.

Run all four gates, twice, and confirm identical results. If the two runs differ, a race
remains.
```

**Check:** the count is the same before and after. If it moved, an assertion changed when
only the plumbing should have.

---

## Step 34 — Triangulation, and the cockpit drills

```
Read spec-v2/rootcause.md (§17.4 §17.10), spec-v2/ui.md (§8.4).

Task: make "eliminated" hold everywhere, then rewire the stage drills.

1. §17.4 is the rigor the register exists to demonstrate. For every eliminated entry,
   assert all three:
     - affected items resolved on the worklist — zero open
     - the agent's log shows those actions closed
     - no new instances arriving

   A client clicking through to find open invoices behind an eliminated entry discounts
   the whole register. Fixed-at-source shows its residue with a falling trend, which
   preserves §7.30's rate-not-pool honesty.

2. Rewire the cockpit stage drills. Each stage goes to the screen holding that stage's
   work, pre-filtered to that stage's causes, and NOT filtered by "needs you" — the demo
   needs to show how much the agents have already done.

       Requisition        Requisitions screen              Step 35
       Purchase order     Commitments watch                already correct
       Goods receipt      Worklist, cause = goods receipt
       Invoice            Worklist, causes = duplicate + tax + invoice quality
       Three-way match    Worklist, cause = pricing
       Approval           Worklist, cause = approval
       Payment            Worklist, cause = payment            or a run view

3. Multi-select on the worklist cause filter, needed for the Invoice stage. Worth having
   regardless — but check whether the invoice stage is genuinely simultaneous or
   sequential (an invoice blocks, clears, then fails match). If sequential, a stage
   filter with cause as secondary may be the better drill; report what you find.

4. The "needs you" default stays on the rail entry — a controller opening the worklist
   during close week wants their own list. It is only the cockpit drills that land
   unfiltered.

Run all four gates.
```

**Check:** every cockpit stage lands somewhere with the right filter applied and the full
picture visible, agent-handled items included.

---

## Step 35 — Requisitions

```
Read spec-v2/upstream.md (§18.0 §18.1 §18.2 §18.2.1), spec-v2/rules.md (§13.7).

Task: the requisition pipeline and its five agents.

0. Two carry-overs from the register screenshots:
   a. The consignment entry showed 50 items against ELIMINATED. With triangulation now
      asserted, either that figure changed or it is a historical count. If historical,
      the column heading must say so — a client reading "items" beside ELIMINATED reads
      it as open, and §17.4's whole point is that clicking through finds nothing.
   b. "Root causes closed" on the mechanism panel is a fifth word alongside the four
      states. It equals eliminated plus fixed-at-source, which is right, but the panel
      carrying the mechanism argument should say so rather than leave it inferred.

1. Read §18.0 before building. This is NOT a worklist. A PR is not an exception — it is
   work in flight taking too long to convert. The screen is closer to Commitments watch:
   a pipeline with ageing and chase state, not an exception queue.

2. Pipeline data per §18.1. PRs in flight are the §7.4 PR stage counts; converted equals
   the PO stage count; the difference is the unconverted pool, which is the subject.
   Assert both ties.

3. The PR-stage cause set in §18.1 is distinct from §6.1 — budget, approval,
   completeness, sourcing, catalogue, duplicate. §6.1 describes exceptions downstream;
   these describe why a PR is not moving. Do not conflate them.

4. Five agents per §18.2, taking the roster from 22 to 27. Four are preventive.

   Agent 23 CONSUMES the Spend Control Tower, it does not replace it. SCT performs the
   budget availability check — that is its control. FCT reads the verdict and owns the
   finance consequence: which PRs will not convert, what that does to committed spend,
   who reallocates. Tag the source on screen the way reconciliation data is tagged.

   Read §18.2.1 — the ecosystem split. Where a control exists, FCT reads it; where none
   exists, FCT owns it. That is why close orchestration came in scope and budget
   availability does not.

   Agents 24 and 26 prevent exceptions that surface three stages later: contract routing
   prevents the price mismatch at three-way match (22% of blocked AP), duplicate PR
   prevents the duplicate invoice at INV. Make that visible on the screen where it is
   true — it is the compounding argument made concrete.

   None of the five creates a commitment. A PR converts when a human decides it should.

5. Wire the P2P Requisition stage drill here. Register fully: routes x4, palette,
   verify-drills, verify-theme.

Run all four gates.
```

**Check:** the unconverted pool ties to PR minus PO for all six entities. Four of the five
agents show as preventive on the roster and the coverage strip.

---

## Step 36 — The O2C worklist

```
Read spec-v2/upstream.md (§18.3 §18.4 §18.5), spec-v2/data-detail.md (§7.20).

Task: the O2C worklist, and the stage drills that have been landing on Working capital.

0. Two carry-overs from Step 35:

   a. The cash-attribution band click failed once on dark/item19 and passed on re-run.
      Treat that as a live race, not a flake — two consecutive passes do not clear an
      intermittent, and it is the only check known to have survived the Step 33b settle
      engine. Find out whether that click bypasses clickOn or goes through it and still
      races. Ten minutes now beats it failing in front of someone.

   b. Renaming "Items" to "Affected items" was right but does not answer the question:
      does the consignment entry marked ELIMINATED still show 50 affected items? §17.4
      says eliminated means nothing open behind it, and Step 34 asserted that per entry.
      So either the number changed, or the column shows historical totals — and
      "Affected items" reads as current either way. Report what the four eliminated
      entries show, and make the column unambiguous.

1. The pool is o2cExceptionCount from §7.2 — 888 across the group, JGL 284. Same grammar
   as the blocked-invoice worklist: value-ranked, cause-filterable, multi-select, agent
   lane, evidence trail, actions. Same §7.20 header, with denominators following the
   filter.

2. Causes are the §6.1 O2C taxonomy unchanged — billing, pricing, deduction, dispute,
   collection, cash application, customer master.

3. Wire the seven O2C stage drills per §18.4. Every one currently lands on Working
   capital, which holds none of that stage's work.

4. Then close the three things that have been open because no O2C worklist existed:
     - cause elimination O2C rows, sent to a cockpit anchor since Step 14
     - root cause register O2C Items figures, left as plain numbers in Step 34
     - §17.10's drill chain, incomplete for O2C

   Each of those was correctly deferred with an honest note. Remove the notes as you
   close them; a limitation that no longer applies is a stale disclosure.

5. Do NOT touch Working capital in this step. Once the O2C worklist absorbs the stage
   drills, that screen holds only ageing buckets and DSO/DPO — but whether it survives
   depends on what remains, and that is visible only after this lands.

Run all four gates.
```

**Check:** every stage in all three cockpits now drills to a screen holding that stage's
work. Nothing lands on Working capital by default.
