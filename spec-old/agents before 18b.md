...
Label it honestly. Every agent surface carries `simulated · demonstrates the operating
model, not a live agent`. A demo that overstates autonomy loses a regulated pharma
client faster than one that underclaims.
...
**Eighteen roles, nine built.** The number matters: eighteen reads as a designed
workforce, twenty-five reads as a list and invites "you've built twenty-five agents?"
Every card carries its status — `live` (working in this prototype), `designed`
(specified, not built) — and the honesty is what makes the coverage credible.
