...
§7.3's cash opportunity says *"release invoices where GR posted this week — ₹4.2 cr,
38 items, Low effort"* — the same cause, at Low. Both cannot be right, and the
consequence is that the "38 resolvable today" chip filters to a set that can never
contain them.

Effort depends on whether the **blocker has already cleared**:

| Situation | Effort | Why |
|---|---|---|
| Missing GR, receipt still not posted | High | Chase the plant, wait for someone else |
| Missing GR, receipt posted this week | Low | Nothing to chase — release it |
...
Store `blockerCleared: boolean` on the exception and derive effort from cause plus
that flag. The 38 resolvable items are those with `blockerCleared: true` — which is
exactly what "GR posted this week" means.
...
```
12 of 327 shown   ·   ₹12.77 cr of ₹18.6 cr   ·   4 of 38 resolvable in this view
```
