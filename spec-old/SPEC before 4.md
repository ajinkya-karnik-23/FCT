...
- **Every point stays within ±25% of current.** A metric must not appear to have
  materialised out of nothing three periods ago.
- **The series must not contradict a stated recurrence.** JGL's missing-GR cause has
  recurred for five months, so JGL's AP blocked series cannot show a clean five-month
  decline from a low base. Assert this: where a cause states recurrence of N months,
  the related metric must be elevated across those N points.
