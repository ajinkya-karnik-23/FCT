...
**On subagents.** Worth using for one thing only: a wide scan that would otherwise fill
the main context. Step 16's consistency check read every screen to verify one figure and
found five hardcoded values; Step 21b swept 630 drill checks and found eight vacuous
probes. Both needed to open many files and neither needed the result kept afterwards —
exactly what a throwaway window is for.

Do **not** use one for a general "quality pass". Two reasons. The gates already do
deterministic QA and have caught more real defects than any review would — white text on
the amber accent at 2.03:1, eight breadcrumb probes matching the rail, a table
overflowing under the drawer. And an open instruction to find improvements produces
plausible changes outside the step, which is what rules 1 and 3 exist to prevent. The
four checks in the primer are the scoped version, and they are scoped deliberately.

The phrasing that works: *"open a subagent, read every file under src/pages, and report
only where <specific figure> is rendered and whether it comes from the dataset"*. Not
*"review my work"*.
...
1. §1's exclusion list drops from three items to two. Close orchestration is no longer
   ...
   Use a subagent for the search: the old wording is scattered across screen copy,
   DEMO.md, README.md and possibly test assertions, and finding it means reading many
   files whose contents you do not need afterwards. Ask it for locations and exact
   strings only — then make the edits yourself in the main window.
...
5. Refresh PROJECT-MAP.md. Four new screens and a new data module have landed since it
   was last written. Use a subagent to re-read the tree and report the inventory; write
   the file yourself from what it returns.
