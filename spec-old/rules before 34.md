...
### §13.7 Browser verification, and the traps in it

`verify-drills.mjs` and `verify-theme.mjs` drive a real browser. Six multi-hour debugging
sessions in this build came from checks assuming a synchrony the browser does not provide.

**Use the helpers, never hand-roll a probe.** `goTo`, `clickOn`, `waitForText`,
`readAttr`. Each waits for the app to commit and fails loudly on timeout with the
selector it was waiting for. A hand-rolled probe is a bug waiting to be written — one in
Step 33 had its own coordinate error and cost hours before anyone noticed the probe was
wrong rather than the app.

**A URL change is not a render.** React commits after the route flips — measured at
~130ms. Comparing `pathname` also cannot see a query parameter dropping, which is how a
filter reset went undetected.

**Rects inside `<defs>` are always zero.** `getBoundingClientRect` on a clip path or
gradient returns nothing useful. Attribute values are the truth.

**Two escaping traps, each of which has cost a session:**
- `\\d` inside a direct regex literal matches a literal backslash-d. Use it only in
  strings passed to `new RegExp()`.
- Never nest a template literal inside `evaluate()`. Build the string node-side and
  inject it with `JSON.stringify`.

**Scope every query.** The rail carries every screen label, so an unscoped text match can
satisfy itself against navigation rather than content. Eight breadcrumb checks passed
that way for twenty steps.
