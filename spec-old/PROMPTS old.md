...
## Step 1 — Central dataset module

```
Read SPEC.md sections §6 and §7.

Task: create a single dataset module and move all hardcoded data into it.

1. Create src/data/controlTower.(ts|js) — match the language the project already
   uses.
2. Define the types/typedefs in SPEC.md §6 exactly as written.
3. Populate the dataset with the canonical figures in SPEC.md §7:
   - six entities with dimension scores (§7.1) and metrics (§7.2)
   - previous-period values for the trend fields listed in §7.2
   - JGL forecast anchors and drivers (§7.3)
   - P2P and O2C stage counts (§7.4)
   - root causes (§7.5)
   - service metrics (§7.7)
   - control signals (§7.8)
   - compliance items (§7.9)
4. Export pure helper functions:
   - computeScore(entity)      → applies weights, then veto caps, returns
                                 { raw, displayed, band, cappedBy }
...
5. Replace hardcoded data in existing components with imports from this module.
   Do not change any component's layout or styling in this step.

Acceptance criteria — the app must still render identically except for the entity
names, and these assertions must hold:
   ...
   computeScore(JRP).displayed === 55 and cappedBy is the bank-change veto
   groupScore() === 76
   valueAtRisk() === 100.4

Add these as a small test file or as console assertions run at module load in dev.
Report the computed values back to me before finishing.
```

**Check:** the assertions pass. If any score is off, the dimension scores or
weights were mistyped — fix the data, not the formula.
...
1. Remove Jubilant Ingrevia and Jubilant Life Sciences NV everywhere. They are not
   part of this group. Jubilant Ingrevia is a separately listed company.
...
Files: the Group view component, the Entity health component, and one new shared
DimensionBar component. Nothing else.
```
...
1. Create a shared Metric component that renders: label, value, delta vs prior
   period, and a small inline sparkline of the last 6 periods.
...
4. Draw the sparkline as an inline SVG. Do not add a charting library.
...
2. Add row actions: Assign, Chase, Release. Add multi-select with bulk versions of
   ...
   evidence array.
3. Add an evidence trail: clicking a row opens a panel showing the item's evidence
   array as a chronological list, plus its attribution and the reason for it.
...
Do not change the visual style of the table. Files: the Worklist component and
the dataset module only.
```
...
3. An SLA table from §7.7: SLA, target, achieved, breaches, attribution split.
...
5. Add the screen to the left rail under a SERVICE group.
```
...
6. These pages are reached by drill only — from the worklist, root cause and
   process screens. Do not add them to the left rail.
...
Task: build the conversational answer panel. This is the single most persuasive
screen in a demo, so it must feel real and must never assert anything the dataset
does not contain.

1. Replace the current "Ask the cockpit" button with a slide-over panel.
2. Four modes: ask, explain, investigate, recommend.
3. Hardcode answers for the six seed questions in §11, computed from the dataset
   where possible rather than written as static strings.
4. Every answer cites its sources as clickable chips — transaction ids, owners,
   service records — that drill to the relevant screen.
5. Every answer offers 2 to 3 follow-up actions.
6. If a question has no answer in the dataset, say so plainly. Never fabricate.
7. Add a visible footer to the panel: "Answers resolve to transactions, owners and
   service records. Nothing is asserted without a source."

Do not integrate a real LLM. This is a scripted demo panel.
```
...
6. Run a consistency check across every screen and report back:
