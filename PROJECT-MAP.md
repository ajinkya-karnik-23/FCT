...
Inventory of the existing codebase (written at Step 0, refreshed at Step 16, re-verified against source at Phase 2 Step 0 on 2026-09-05, refreshed at Step 28 from `git diff f571103..HEAD -- src/` plus the working tree). No source files were modified by this document.
...
All fabricated reference data is under `src/api/mock/*.ts` (16 modules), exposed only through the synchronous accessors in `src/api/index.ts`. Components import from `../api` (or `../../api`) and never from `./mock/*` directly. Domain types are declared in `src/api/types.ts`. There is no network layer — every accessor returns a static array/object.
...
- `src/App.tsx` — plays the logo intro once per browser session (sessionStorage gate, never on route changes), then wraps `<AppShell>` in `BrowserRouter`.
...
- `src/app/Rail.tsx` — left navigation rail (236px): brand block, nav items grouped under the seven §9.1 labels (OVERVIEW · PROCESS · EXPLAIN · ASSURE · FORWARD · SERVICE · AGENTS) with counts, search trigger (⌘K). Counterparty pages are drill-only and never appear here.
...
- `src/app/routes.tsx` — route table (`AppRoutes`), breadcrumb builder, active-nav resolver, and `RAIL_GROUPS` + `NAV_ITEMS` (19 items; counts derive from API accessors, no literals).
...
- `src/theme/clay.ts` — shared surface recipes built from tokens (`card`, `frame`, sunken wells, tags, mono notes): elevation spent by role; introduces no colours or shadows of its own.
...
- `src/api/mock/stages.ts` — P2P stages (7), O2C stages (7) and R2R stages (8, §16.2: SUB → SGN): step code, name, in-flight volume/value, exception count; JGL is the base table, every other entity scales it (§7.25) then pins its own tie points to its metrics.
...
- `src/api/mock/agents.ts` — §15 agent workforce: twenty-two roles (ten live), deterministic per-agent metrics, value caps and action logs seeded on stable keys; the lifecycle coverage strip (§15.2.0) positions agents across P2P/O2C/R2R stages with gaps left visible.
- `src/api/mock/touchEconomics.ts` — §15.4 per-entity touch funnel: touchless/manual split with agent-vs-human resolution and touches per 1,000 today vs after; every row reconciles by construction.
- `src/api/mock/commitments.ts` — §15.7 commitments watch: open POs by delivery date (the named rows are the cost-centre pool), chase/amendment state and value at risk of slipping past period-end; exchange timelines seeded on stable keys against the anchor clock.
- `src/api/mock/closeCalendar.ts` — §16.3 close calendar: per-entity task-pool summaries pinned by the spec table (reconciling to §7.2's close %) plus the named blocked/critical-path tasks with owners, due days, dependencies, blockers and escalation state; predicted day is the critical path's furthest due day.
- `src/api/mock/balanceSheet.ts` — §16.4–§16.5: integrity weights + per-entity components (the index is computed at read time), journal-risk population with its seven flags, intercompany counterparties, accrual reversals and recon summary with overdue-breaks ageing.
...
- `SplashScreen.tsx` — logo intro that plays once per browser session (gated in `App.tsx`), never on route changes.
...
- `CrossProcessTrace.tsx` — §8.10 four-node chain (missing goods receipt → blocked invoice → understated accrual → close exposure); the current node renders as a bordered span, the other three as links; rendered at all four points, and the chain's end lands on the R2R cockpit (§16.7).
...
- `R2RCockpit.tsx` — §16.2/§16.4–§16.5 Level 2 R2R process view: eight stage cards (SUB → SGN), balance-sheet integrity index (six weighted components, computed at read time) and the four panels (reconciliations, journal risk, intercompany, accruals & provisions); cross-process trace from the close node; renders in all three cockpit modes.
- `CloseCalendar.tsx` — §16.3 per-entity close calendar: KPI row (close %, committed vs predicted day, open tasks, blockers), blocked-first attention slice with critical-path dots and blocker/escalation columns, sign-off status only.
...
- `CommitmentsWatch.tsx` — §15.7 drill-only commitments watch under P2P: open POs by delivery date with chase state and value at risk of slipping past period-end; the preventive agent's claim card.
- `PoDetail.tsx` — §15.7 drill-only PO detail (terminal): the nine-day exchange timeline (agent/owner messages) plus the decision record with its confidence threshold.
- `RootCause.tsx` — Level 5 process-aware root cause: taxonomy selector driven by route params (`p2p`/`o2c`/`r2r`), primary-cause narrative + metrics, plant/segment and vendor/driver splits (plant rows drill sideways to counterparty pages), recommended interventions; cross-process trace from the goods-receipt node when `missing-gr` is selected.
...
- `CashAttribution.tsx` — where cash is stuck across both processes, grouped by the function that causes it rather than the one that holds it (group-scoped).
...
- `Agents.tsx` — §15 agent workforce roster: role cards with live/specified status, delegation fields and per-agent metrics; the lifecycle coverage strip positions agents across P2P/O2C/R2R stages with gaps left visible. Rail badge is the live count.
- `AgentDetail.tsx` — single-agent record (drill-only under /agents/:agentId): decision records with checks and precedents, action log, reversibility and override control.
- `TouchEconomics.tsx` — §15.3/§15.4 the commercial conversation: per-entity touch funnel (touchless/manual split, touches per 1,000 today vs after) with the commit stated as touches, not an automation percentage.
...
- `tests/` — 22 Vitest suites: `api.test.ts`, `app-shell.test.tsx`, `drill-paths.test.tsx`, `o2c-cockpit.test.tsx`, `p2p-worklist-exception.test.tsx`, `pages-group-entity.test.tsx`, `palette-assistant.test.tsx`, `rootcause-workingcapital.test.tsx`, `routes.test.ts`, `risk-control.test.tsx`, `predictive.test.tsx`, `counterparty.test.tsx`, `assure-compliance-dq.test.tsx`, `service-attribution.test.tsx`, `service-desk.test.tsx`, `cause-backlog.test.tsx`, `cross-process-trace.test.tsx`, `agents.test.tsx`, `touch-economics.test.tsx`, `commitments.test.tsx`, `r2r-cockpit.test.tsx`, `close-calendar.test.tsx`.
