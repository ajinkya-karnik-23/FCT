...
- The worklist header leads with the agents' output, before any row:
  `327 blocked · 241 resolved by agents · 86 need you`.
...
**Two end states, and they must be labelled apart.** §15.4's per-entity table applies
agents to *today's* exception volume — JGL lands at 13.8% human touch, 138 per thousand.
This section adds the first lever: eliminate causes, then work the smaller residue —
114 per thousand. Both are correct and the difference between them is the whole point,
but shown side by side with no distinction they read as an error.

```
138 per 1,000   agents alone, on today's exception volume
114 per 1,000   causes eliminated first, then agents on the residue
```

On screen, label them exactly that way. The funnel shows the first; the lever
progression shows the second.
